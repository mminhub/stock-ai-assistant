...
if "GOOGLE_API_KEY" not in st.secrets:
    ...
    st.info("Streamlit Cloud에서 Settings -> Secrets 에 GOOGLE_API_KEY를 추가해주세요.")
    st.stop()
    ...
    for model in RELAY_MODELS:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                # 429가 아닌 다른 에러는 로그에 기록
                error_msg = f"[{model}] Error {res.status_code}: {res.text}"
...
def main():
    ...
    if 'briefing_data' not in st.session_state:
        ...
        news_txt = "\n".join([f"[{i+1}] {n.title} ({n.get('published', 'No Date')})" for i, n in enumerate(news)])
        ...
        if ai_res:
            ...
            st.code(error_log) # 화면에 빨간 박스로 에러 내용이 뜹니다.
            st.stop()
    ...
    for i, n in enumerate(news):
        ...
        with st.container():
            ...
            st.caption(f"🕒 {n.get('published', '')} | 💡 {rsn.strip()}")
